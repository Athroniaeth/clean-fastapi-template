...
class APIKeyRepository:
    ...
    async def get_by_id(self, key_id: int) -> Optional[ApiKeyModel]:
        ...
        Args:
            key_id (int): primary key of the API key record.
    ...
    async def get_by_hashed_key(self, hashed_key: str) -> Optional[ApiKeyModel]:
        ...
        This can be useful when verifying an incoming raw key: first hash it,
    ...
    async def save(self, api_key: ApiKeyModel) -> None:
        """
        Persist changes made to an existing ApiKeyModel.

        Notes:
            After modifying fields on the instance, call this to commit updates.

        Args:
            api_key (ApiKeyModel): the instance with updated attributes.
        """
        # The instance is already in the session; just flush changes
        await self._session.flush()
